...
# 命令为：python train.py --model convnext --gpu_id "3,4,5"

# 如果传入参数，则使用传入的 conda.sh 路径；否则使用默认路径
CONDA_SH_PATH=${1:-"/data/private/hzp/app/anaconda3/etc/profile.d/conda.sh"}
gpu_id=${2:-""}

echo "使用的 conda.sh 路径为：$CONDA_SH_PATH"
echo "使用的 GPU ID：$gpu_id"
...
for model in "${models[@]}"; do
    ...
    # 第一次执行训练脚本，传入当前模型名称
    python train.py --model "$model" --gpu_id "$gpu_id"
    status=$?
    ...
    if [ $status -ne 0 ]; then
        ...
        python train.py --model "$model" --resume "true" --gpu_id "$gpu_id"
    ...
    if [ $status -eq 0 ]; then
        ...
        python test.py --model "$model" --gpu_id "$gpu_id"
